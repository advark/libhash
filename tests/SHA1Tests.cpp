...
void libHashTestCases::testSHA1( ) {
    ...
    for( int i = 0; i < sizeof ( testCases ) / sizeof ( TestData ); i++ ) {
        runSingleChunk( i + 1, sha1, "SHA-1", testCases[i].data, testCases[i].size, testCases[i].sha1 );
    }

    for( int i = 0; i < sizeof ( testCases ) / sizeof ( TestData ); i++ ) {
        runMultiChunk( 11 + i, sha1, "SHA-1", testCases[i].data, testCases[i].size, ( testCases[i].size * 0.13 ) + 1, testCases[i].sha1 );
    }
}
